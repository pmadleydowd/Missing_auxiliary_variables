cap log close 
log using "$Logdir/Combine_results/log_collate_sensitivity_results.txt", text replace
********************************************************************************
* Author: 		Paul Madley-Dowd
* Date: 		27 Oct 2022
* Description:	Collate results from all sensitivity simulations 
********************************************************************************
* Contents 
********************************************************************************
...
foreach sens in More_missing_outcome Stronger_ZY_than_XY {
	foreach outmis in /*MAR_outcome Proxy_outcome*/ Efficiency_imputation {
		if "`outmis'" == "Efficiency_imputation" {
			local auxmis = "3_MNAR_auxiliary" 		
		}
		else {
			local auxmis = "2_MAR_auxiliary" 		
		}
		...
		forvalues sim = 1(1)1000	{
			forvalues auxid = 1(1)4 {
				...
				disp "outmis = `outmis', sensitivity  = `sens', sim = `sim', auxid = `auxid'"
				use "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/simout_sim`sim'_auxid`auxid'.dta", clear
				gen seq = _n
				...
				save "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data/_temp/sim`sim'_auxid_`auxid'.dta", replace
...
foreach sens in More_missing_outcome Stronger_ZY_than_XY {
	foreach outmis in /*MAR_outcome Proxy_outcome*/ Efficiency_imputation {
		...
		forvalues auxid = 1(1)4 {
			use "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data/_temp/sim1_auxid_`auxid'.dta", clear
			forvalues sim = 2(1)1000	{
				append using "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data/_temp/sim`sim'_auxid_`auxid'.dta"
			}
			rename se_* beta_*_se // used for simsum command
			save "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data/simall_auxid_`auxid'.dta", replace
			
			forvalues sim = 1(1)1000	{
				capture erase "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data/_temp/sim`sim'_auxid_`auxid'.dta"
			}
		}
	}
}		
...
foreach sens in More_missing_outcome Stronger_ZY_than_XY {
	foreach outmis in /*MAR_outcome Proxy_outcome*/ Efficiency_imputation {
		...
		forvalues auxid = 1(1)4 {
			...
			use "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data/simall_auxid_`auxid'.dta", clear
			...
			save "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data/_temp/simsum_out_`auxid'.dta", replace

			drop if mcse==1 
			drop bias_mcse emp_se_mcse modelse_mcse relerror_mcse cover_mcse mcse
			save "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\simsum_out_beta_`auxid'.dta", replace
			use "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\simsum_out_`auxid'.dta", clear
			drop if mcse==0
			drop bias emp_se modelse relerror mcse
			save "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\simsum_out_mcse_`auxid'.dta", replace
			
			
			use "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\simsum_out_beta_`auxid'.dta", clear
			merge 1:1 varname using "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\simsum_out_mcse_`auxid'.dta", nogen
			order varname 
			...
			save "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\simsum_out_`auxid'.dta", replace
		}
	}
}	
	...
	foreach outmis in /*MAR_outcome Proxy_outcome*/ Efficiency_imputation {
		...
		forvalues auxid = 1(1)4 {
			...
			postfile `memhold' auxid auxcorr auximp misaux mean se median LQ UQ /// 
					 using  "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\FMI_sumstats_auxid`auxid'.dta", replace
	...
	foreach outmis in /*MAR_outcome Proxy_outcome*/ Efficiency_imputation {
		...
		forvalues auxid = 1(1)4 {
			...
			postfile `memhold' auxid auxcorr aux imp misaux mean se median LQ UQ /// 
					 using  "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\prop_yzmiss_sumstats_auxid`auxid'.dta", replace
...
foreach sens in More_missing_outcome Stronger_ZY_than_XY {
	foreach outmis in /*MAR_outcome Proxy_outcome*/ Efficiency_imputation {
		...
		forvalues auxid = 1(1)4 {
			...
			use "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\prop_yzmiss_sumstats_auxid`auxid'.dta", clear
			merge 1:1 misaux aux imp using "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\simsum_out_`auxid'.dta", keep(3) 
			save "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\prop_yzmiss_sumstats_auxid`auxid'.dta", replace
			capture erase "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/Prepared_data\_temp\prop_yzmiss_sumstats_auxid`auxid'.dta"
