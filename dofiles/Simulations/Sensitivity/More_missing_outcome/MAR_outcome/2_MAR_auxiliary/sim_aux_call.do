...
forvalues sim = 1(1)$simno {
	...
	* Simulate missing data in Y 
	gen Y_m75 = Y if normal(Z-2) > sqrt(0.75) | normal(X+3) > sqrt(0.75)



	* Simulate missing data in the auxiliaries 
	forvalues j = 0(1)9 {
		gen Z_`j' = Z if normal(W) > `j'/10 
	}
	...
	* run models 
	...
	tempname memhold 
	postfile `memhold' sim auxid auxcorr full cca imp aux misprop beta se fmi prop_yzmiss using "$Datadir/Sensitivity/`sens'/`outmis'/`auxmis'/simout_sim`sim'_auxid`auxid'.dta", replace
	...
	preserve
		mi set flong 
		mi register imputed Y_m75  
		mi register regular X 

		mi impute chained (regress) Y_m75 = X, add($impno) dots
		mi estimate: regress Y_m75 X 
	...
	* Perform imputations
	forvalues j = 0(1)9 {
		...
		mi set flong 
		mi register imputed Y_m75 Z_`j'  
		mi register regular X W

		mi impute chained (regress, omit(W)) Y_m75 ///
						  (regress) Z_`j' ///
						  = X W, add($impno) dots
		mi estimate: regress Y_m75 X 
